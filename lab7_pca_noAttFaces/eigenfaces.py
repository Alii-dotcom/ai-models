...
class Eigenfaces:
    ...
    def fit(self, X: np.ndarray, verbose: bool = False):
        ...
        if verbose:
            # show mean face
            plt.imshow(np.reshape(self._x_mean, shape=(112, 92)), cmap='gray')
            plt.title('mean face')
       ...
       #sorted_eigval, sorted_eigvec = np.sort(eigval)[::-1], np.sort(eigvec)[:, ::-1]
        ...
        self._ef = (X_norm.T @ principal_sorted_eigvec) / np.sqrt(principal_sorted_eigval)        # normalize the retrieved eigenvectors to have unit length
